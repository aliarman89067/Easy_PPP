...
export default function Page() {
  return (
    <>
      <section className="min-h-screen  bg-[radial-gradient(hsl(0,72%,65%,40%),hsl(24,62%,73%,40%),hsl(var(--background))_60%)] flex items-center justify-center text-balance text-center flex-col gap-8 px-4">
        <h1 className="text-6xl lg:text-7xl xl:text-8xl font-bold tracking-tight mt-4">
      ...
      <section id="pricing" className="px-8 py-16 bg-accent/5">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 max-w-screen-xl mx-auto">
          {subscriptionTiersInOrder.map((subscription) => (
            <PricingCard key={subscription.name} {...subscription} />
          ))}
        </div>
      </section>
...
const PricingCard = ({
  ...
  return (
    ...
    >
      {isMostPopular && (
        <div className="bg-accent text-white absolute py-1 px-10 -right-8 top-24 rotate-45 origin-top-right">
          Most Popular
        </div>
      )}
      <CardHeader>
